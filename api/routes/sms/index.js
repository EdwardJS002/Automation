...
router.get(
	...
	function (req, res) {
		...
		try {
			...
			if (phoneNumberListParsed.length <= 0) return res.status(400).json({ error: 'Any Good Number on the list' }).end()

			phoneNumberListParsed.map((element) => sendsms(element, message))

			return res.status(201).json({ res: 'Message is sent to all numbers' }).end()
		} catch (error) {
